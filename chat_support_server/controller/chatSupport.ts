...
const login = async (req, reply) => {
  ...
  if (!user) {
    return errorResponse('Invalid username or password', reply, 'custom');
  }
  ...
  try {
    ...
    return errorResponse('Authentication error', reply, 'custom');
  }

  // 3. Update user's isActive status to true
  try {
    await User.update({ isActive: true }, { where: { id: user.id } });
    user.isActive = true; // Update the local user object to reflect the change
  } catch (err) {
    console.error('Error updating user status:', err);
    return errorResponse('Login processing error', reply, 'custom');
  }
